import { doc, setDoc, getDocs, collection, query, where, getDoc, updateDoc, arrayUnion, arrayRemove, addDoc} from "firebase/firestore";
...
export class PostService {
    static async createPost(
        ...
        try {
            ...
            return postRef.id; // Retorna el ID generado por Firestore
        } catch (error) {
    ...
    private static async uploadToImgBB(file: any) {
    ...
    static async setImageToPost(postId: string, file: any): Promise<string> {
        try {
            ...
            const postRef = doc(db, 'posts', postId);
            await setDoc(postRef, { imageUrl: imageUrl }, { merge: true });
            return imageUrl;
        } catch (error) {
